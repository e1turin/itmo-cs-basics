...
chmod u+r $cacturne3 
ln -s ../cacturne3 $phanpy0/bronzorcacturne
#chmod u-r $cacturne3
#chmod u-w $phanpy0 # will do it later
...
cp $cacturne3 $phanpy0/bronzorcacturne 2>/dev/null 
...
ls -lR $lab0 | /usr/xpg4/bin/grep -E "^[-dl][-rwx]{7}.* t.*$" | /usr/xpg4/bin/tail -n 3 | sort -k 2
...
cat -n $cacturne3  | /usr/xpg4/bin/grep -vi "d$"
...
ls -lRtr $lab0 2>/dev/null | /usr/xpg4/bin/grep -E "[-dlr][-rwx]{7}.*swa.*" 
...
ls -lRt $lab0 2>/tmp/err.log | /usr/xpg4/bin/grep "r$" 
...
chmod u+r $feraligatr5
rm $feraligatr5/fraxurecacturne
